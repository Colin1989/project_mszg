...
make_protocal_def() ->
    Structures = protocal_def:get_struct_def(),
    StructureStr = get_structure(Structures, ""),
    MessageStr = create_defines(Structures, 1, ""),
    VersionStr = create_version(protocal_def:get_version()),
...
merge_record(Type, Name, Str, Flag) ->
    merge_record(Type, Name, 
		 get_default_value(gen_common:get_default_table(), 
				   Type, 
				   gen_common:get_enum_def_list(protocal_def:get_enum_def())), Str, Flag).
...
make_file() ->
    ...
    Encode = make_function_encode(protocal_def:get_struct_def(), "", gen_common:get_type()),
    Decode = get_struct_member(protocal_def:get_struct_def(), "", gen_common:get_type()),
...
make_function_serilize_arg({pack_type, _Type}, VarName) ->
    "(protocal:encode(" ++ string:to_upper(atom_to_list(VarName)) ++ "))/binary";
