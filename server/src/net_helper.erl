...
-export([packet_encode/1, packet_decode/2, make_net_binary/1, make_net_binary/4]).
...
get_data(RawBinary, WantPacketCount) ->
    {Type, Rest, LastRecv} = packet_decode(RawBinary, WantPacketCount),
    {Data, _Bin} = protocal:decode(Type, Rest),
    {{Type, Data}, LastRecv}.
...
%% 生成网络消息的二进制数据
make_net_binary(Type, null, MyStep, RecvStep) -> %只有网络类型, 不需要结构体数据的网络消息
    net_helper:packet_encode(<<MyStep:?UCHAR, RecvStep:?UCHAR, Type:?UINT16>>);
make_net_binary(Type, Binary, MyStep, RecvStep) ->
    net_helper:packet_encode(<<MyStep:?UCHAR, RecvStep:?UCHAR, Type:?UINT16, Binary/binary>>).
...
packet_decode(Binary, WantPacketCount) ->
    ...
    <<LastRecv:?UCHAR, WantPacketCount:?UCHAR, Type:?UINT16, Rest/binary>> = RawBinary,
    {Type, Rest, LastRecv}.
