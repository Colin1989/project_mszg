    ...
    lists:foreach(fun(X) ->
        ...
        case redis:hget(rank_bak_done_status, Key) of
            ...
            _ ->
                ...
                redis:del(lists:concat([X, "_topn_bak"]))
        end
...
get_rank_top_n(RankType, N) ->
    %%{Year, Month, Day} = get_rank_date(?bak_time),
    %%Key = format_key(RankType, Year, Month, Day),
    case redis:get(lists:concat([RankType, "_topn_bak"])) of
    %%case redis:get(lists:concat([Key, "_bak"])) of
        undefined ->
            get_rank_top_n_from_cache(RankType, N);
        RankInfo ->
            RankInfo
    end.
...
get_rank_top_n_from_cache(RankType, N) ->
    {Year, Month, Day} = get_rank_date(?bak_time),
    Key = format_key(RankType, Year, Month, Day),
    PreList = redis:zrevrange(Key, 0, N - 1, true),
    List = lists:reverse(lists:keysort(2, PreList)),
    ...
    ReturnList = lists:reverse(RResult),
    redis:set(lists:concat([RankType, "_topn_bak"]), ReturnList),
    ReturnList.
