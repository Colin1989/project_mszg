...
call({?msg_req_login_check, #req_login_check{uid = Uid, token = Token}})->
    {obj, ResponseList} = http_client:request_with_jason_encode([{app_id, config:get_server_config(app_id)}, 
								 {app_key, config:get_server_config(app_key)}, 
								 {server_id, config:get_server_config(server_id)}, 
								 {token, Token}, {uid, Uid}]),
	...
	true ->
	    ...
	    init_player_info(User),
            ...
            {ChannelID, _Rest} = string:to_integer(binary_to_list(proplists:get_value("message", ResponseList))),
            redis:hset("channel_id", User:user_id(), ChannelID),
            redis:hdel("tcp_msg_queue", User:user_id()),
	    try 
	...
	false ->
	    ErrorCode = list_to_integer(binary_to_list(proplists:get_value("code", ResponseList))),
	    case ErrorCode of
		2007 ->
		    sys_msg:send(self(), ?sg_reconnect_server_not_running, [binary_to_list(proplists:get_value("message", ResponseList))]);
		_ ->
		    ok
	    end,
...
call({?msg_req_select_role, #req_select_role{role_id = RoleId} = Packet})->
	...
	[Role] ->
	    ...
	    case Role:status() of
		?role_status_normal->
		    case player_role:check_role_in_status(Role, ?rpc_RoleStatus_KICK) of
			false ->
			    packet:send(self(), #notify_select_role_result{result = ?common_success}),
			    {ok, Pid} = player:start_link(self(), Role:user_id(), Role, get(user_info)),
			    put(player_pid, Pid),
			    {next_state, wait_for_data};
			true ->
			    packet:send(self(), #notify_select_role_result{result = ?common_failed}),
			    sys_msg:send(self(), ?sg_select_role_locked, []),
			    {next_state, wait_for_auth}
		    end;
		_ ->
...
call({?msg_req_delete_role, #req_delete_role{role_id = RoleId} = Packet})->
	...
	[{Role, _}] ->
	    case (Role:status() =:= ?role_status_normal) andalso (not player_role:check_role_in_status(Role, ?rpc_RoleStatus_KICK)) of
		true ->
		    process_delete_role(Roles, Role),
                    invitation_code:disengage_by_del_role(Role:role_id()),
...
call({?msg_req_register, #req_register{account = Account, channelid = ChannelId, password = Password, platformid = PlatformId} = Packet}) ->
	...
	case has_registered(Account) of
		...
		Id ->
			...
			NewAccount:save(),
			NewUser = db_user:new(id, Id, ?account_status_normal, Password, ChannelId, PlatformId, config:get(init_emoney), 0, datetime:local_time()),
			NewUser:save(),
...
%% @end
%%--------------------------------------------------------------------
call({?msg_req_game_reconnect, #req_game_reconnect{uid=Uid, token=Token, role_id = RoleId ,last_recv_stepnum = ClientLastRedv}})->
	...
	Token ->
	    ...
	    init_player_info(PlayerInfo),
    	    case lists:filter(fun(X) -> X:role_id() =:= RoleId end,db:find(db_role,[{user_id,'equals',PlayerInfo:user_id()}])) of
    		RoleList when length(RoleList)>=1->
		    {LastRecv, Queue} = tcp_mm:get_queue_from_cache(),
    		    packet:send(self(), #notify_reconnect_result{result=?reconnect_success,id=PlayerInfo:user_id(), last_recv_stepnum = LastRecv}),
		    Infos = tcp_mm:get_resend_packets(Queue, ClientLastRedv),
		    %%io:format("QUEUE:~p, LastRecv~p,Infos:~p~n", [Queue, LastRecv, Infos]),
		    tcp_mm:resend_packet(Infos),
    		    [Role]=RoleList,
    		    {ok, Pid} = player:start_link(self(), PlayerInfo:user_id(), Role, get(user_info), not_notify),
    		    put(player_pid, Pid)
    	    end,
    	    {next_state, wait_for_data};
	_ ->
...
%% @end
%%--------------------------------------------------------------------
call({?msg_req_verify_invite_code, #req_verify_invite_code{code  = Code}})->
    case invitation_code:verify_code(Code) of
        {true, _MasterID} ->
            packet:send(self(), #notify_verify_invite_code_result{result = ?common_success});
        false ->
            packet:send(self(), #notify_verify_invite_code_result{result = ?common_failed})
    end,
    {next_state, wait_for_auth}.
...
process_create_role( PlayerID, RoleType, NickName)->
    ...
    FixInitSculpture = lists:filter(fun(X) -> X =/= 0 end, RoleTplt#role_tplt.init_sculpture),
    lists:map(
        fun(X) ->
            Sculpture =
            sculpture_pack:create_sculpture_and_save(?st_create_role, ?item_sculpture, X, uuid:gen(), 1, RoleID),
            Sculpture:temp_id()
        end, FixInitSculpture),
    sculpture:init_sculpture(RoleID, RoleTplt#role_tplt.init_sculpture),

    Role = player_role:create(RoleID, NickName, PlayerID, RoleType, ArmorItem:equipment_id(), 
			      WeaponItem:equipment_id(), Skill1, Skill2),
...
get_db_user_by_userid(UserId) ->
	...
	[] ->
	    NewUser=db_user:new(id, UserId, ?account_status_normal, "", 0, 0, config:get(init_emoney), 0, datetime:local_time()),
