...
-export([date_to_gregorian_days/1, 
	 ...
	 stime_to_seconds/1,
	 timestr_to_time/1]).
-export([get_today_left_second/0,get_week_left_second/0,get_month_left_second/0]).
...
timestr_to_time(TimeStr) ->
    [Year, Month, Day, Hour, Minute, Second] = string:tokens(TimeStr, "- "),
...
stime_to_seconds(#stime{year=Y1, month=M1, day=D1, hour=H1, minute=Min1, second=S1}) ->   
    calendar:datetime_to_gregorian_seconds({{Y1, M1, D1}, {H1, Min1, S1}}).
...
get_today_left_second()->
    calendar:datetime_to_gregorian_seconds({get_next_day(),{0,0,0}}) - 
	calendar:datetime_to_gregorian_seconds( erlang:localtime() ).
get_next_day()->
    {Date,_Time}=calendar:gregorian_seconds_to_datetime(calendar:datetime_to_gregorian_seconds(erlang:localtime())+86400),
    Date.

get_week_left_second()->
    calendar:datetime_to_gregorian_seconds({get_next_weed(),{0,0,0}}) - 
	calendar:datetime_to_gregorian_seconds( erlang:localtime() ).
get_next_weed()->    
    Weekday=calendar:day_of_the_week(erlang:date()),
    {Date,_Time}=calendar:gregorian_seconds_to_datetime(calendar:datetime_to_gregorian_seconds(erlang:localtime())+86400*(8-Weekday)),
    Date.

get_month_left_second()->
    calendar:datetime_to_gregorian_seconds({get_next_month(),{0,0,0}}) - 
	calendar:datetime_to_gregorian_seconds( erlang:localtime() ).

get_next_month()->
    {Year,Month,Day}=erlang:date(),
    FirstDay=calendar:last_day_of_the_month(Year,Month)+1,
    {Date,_Time}=calendar:gregorian_seconds_to_datetime(calendar:datetime_to_gregorian_seconds(erlang:localtime())+86400*(FirstDay-Day)),
    Date.
