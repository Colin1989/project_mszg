...
-export([seed/0, seed/1, seed/3, uniform/0, uniform/1, uniform/2, uniform/3,
	 uniform_s/1, uniform_s/2, seed0/0, rand_members_from_list/2, rand_members_from_list_not_repeat/2]).
...
rand_members_from_list([], _) ->
    [];
rand_members_from_list(List, MemberAmount) when is_list(List) and is_integer(MemberAmount)->
    case MemberAmount of
	0 ->
	    [];
	Amount when Amount>0 ->
	    Member = lists:nth(rand:uniform(length(List)),List),
	    [Member|rand_members_from_list(List, MemberAmount-1)]
    end.

rand_members_from_list_not_repeat([], _) ->
    [];
rand_members_from_list_not_repeat(List, MemberAmount) when is_list(List) and is_integer(MemberAmount)->
    case MemberAmount of
	0 ->
	    [];
	Amount when Amount>0 ->
	    Member = lists:nth(rand:uniform(length(List)),List),
	    [Member|rand_members_from_list_not_repeat(List--[Member], MemberAmount-1)]
    end.
