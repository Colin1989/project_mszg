...
put_roleinfo(RoleInfo) ->
    redis:hset(role_info_detail, player:get_role_id(), RoleInfo),
    put(roleinfo, RoleInfo).
...
remake_roleinfo_detail(Id,Status)->
	...
	undefined -> 
	    ...
	    undefined;
	Role ->
		#skill_group_item{id1 = ID1, id2 = ID2, id3 = ID3, id4 = ID4} = sculpture:get_cur_skill_group_info(Id),
		Sculptures = get_sculpture_tempids(Id, [ID1, ID2, ID3, ID4]),
		io_helper:format("final scrupture:~p~n", [Sculptures]),
	    Equipments = equipment:get_equipments_on_body(Id, equipment:get_equipment(Role)),
	    AttrProp = battle_power:get_role_battle_prop(Role),
	    BattleProp = #role_attr_detail{sculptures = Sculptures, equipments = Equipments, life = AttrProp#battle_prop.life, 
					   speed = AttrProp#battle_prop.speed, atk = AttrProp#battle_prop.atk, hit_ratio = AttrProp#battle_prop.hit_ratio,
		...
		FriendInfo = #role_info_detail{status = Status, type = Role:role_type(), nickname = Role:nickname(),
									   ...
									   potence_level = Role:potence_level(), advanced_level = Role:advanced_level(),
											mitigation = equipment:get_mitigation(Equipments)},
	    cache:set(role_info_detail, Id, FriendInfo),
	    %%io_helper:format("FriendInfo:~p~n", [FriendInfo]),
	    FriendInfo
    end.

%%获取符文ID和等级
get_sculpture_tempids(RoleID, Sculptures)->
    lists:map(fun(X) ->
		      case X of
			  0 ->
			      #sculpture_data{};
			  _ ->
			      sculpture:get_sculpture_tempid_and_lev_by_role_id(RoleID, X)
		      end
	      end,Sculptures).
