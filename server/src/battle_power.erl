...
get_role_battle_prop(RoleInfo) ->
    ...
    BaseProp = get_role_base_battle_prop(RoleInfo:role_type(),RoleInfo:level(), RoleInfo:potence_level()),

    battle_power:battle_prop_info_addition(EuqipmentProp,BaseProp).
    

get_role_base_battle_prop(RoleType,Level, PotenceLevel)->
    Id = RoleType*1000 + Level,
    Info = tplt:get_data(role_upgrad_tplt,Id),
	{PotenceAtk, PotenceLife, PotencePower} = role_advance:get_role_potence_info(PotenceLevel),
	#battle_prop{life = Info#role_upgrad_tplt.life + PotenceLife,
		atk = Info#role_upgrad_tplt.atk + PotenceAtk,
		...
		power = Info#role_upgrad_tplt.combat_effectiveness + PotencePower}.
...
add_equipment_level_battle_prop(Level, StrendId, Prop)->
    NProp = case Level of
		0 ->
		    Prop;
		_ ->
		    StrenBattlePower = equipment:get_tplt_stren_battle_power(StrendId * 1000 + Level),
		    LevProp = equipment:get_equipment_level_battle_prop(StrendId, Level),
		    battle_prop_info_addition(LevProp, Prop#battle_prop{power = Prop#battle_prop.power + StrenBattlePower})
	    end,
