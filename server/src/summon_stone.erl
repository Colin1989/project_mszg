...
notify_boss_copy_fight_time() ->
    Num = case cache_with_expire:get('boss_copy_fight_time', player:get_role_id()) of
        [] ->
            0;
        [{_Roleid, Count}] ->
            Count
    end,
    packet:send(#notify_boss_copy_fight_count{count = Num}).
...
proc_req_daily_summon_stone(#req_daily_summon_stone{}) ->
  ...
  case cache:get(daily_awarded_summon_stone_list, RoleID) of
    [] ->
      ...
      Amount = config:get(daily_award_summon_stone_amount) + vip:get_privilege_count(1),
      player_role:add_summon_stone(?st_summon_stone_daily_award, Amount),
...
proc_req_buy_summon_stone(#req_buy_summon_stone{}) ->
    ...
    case check_buy_summon_stone_enable(HasBuyTimes) of
	true ->
		...
		true ->
		    ...
		    player_role:add_summon_stone(?st_summon_stone_buy, AddCount),
		    player_role:reduce_emoney(?st_summon_stone_buy, NeedEmoney),
	...
	_ ->
	    packet:send(#notify_buy_summon_stone{result = ?common_failed}),
	    sys_msg:send_to_self(?sg_summon_stone_buy_times_exceeded, [])
    end.
...
check_buy_summon_stone_enable(Times) ->
    Times < config:get(summon_stone_max_buy_times).
