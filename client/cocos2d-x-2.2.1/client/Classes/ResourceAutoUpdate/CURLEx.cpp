...
bool CURLEx::setHeaders(const std::vector<std::string>& headers)
{
	...
	for (iter; headers.end() != iter; ++iter)
	{
		// the second parameter must use type: const char*
...
//------------------------------------------------------------------------
bool CURLEx::setPostFields(const std::string& fields)
{
	if (fields.empty())
		return false;

	CURLcode code;
	code = setOption(CURLOPT_POSTFIELDS, fields.c_str());
	if (CURLE_OK != code)
		return false;

	code = setOption(CURLOPT_POSTFIELDSIZE, fields.size());
	return CURLE_OK == code;
}
//------------------------------------------------------------------------
...
bool CURLEx::addForm(curl_forms forms[], unsigned int length)
{
	...
	CURLFORMcode code = curl_formadd(&mPost, &mLast, CURLFORM_ARRAY, forms, CURLFORM_END);
...
//------------------------------------------------------------------------
bool httpReuqestConfigure(CURLEx* pCurl, const HttpResuest& request, CURLEx_callback writeFunc, void* writeStream, CURLEx_callback headerCallback, void* headerStream)
{
...
//------------------------------------------------------------------------
bool httpGet(const HttpResuest& request, CURLEx_callback writeFunc, void* writeStream, CURLEx_callback headerCallback, void* headerStream, int* curlCode, int* responseCode, std::string* errorBuffer)
{
	CURLEx curlObj;
	if (!httpReuqestConfigure(&curlObj, request, writeFunc, writeStream, headerCallback, headerStream))
		return false;
...
//------------------------------------------------------------------------
bool httpPost(const HttpResuest& request, CURLEx_callback writeFunc, void* writeStream, CURLEx_callback headerCallback, void* headerStream, int* curlCode, int* responseCode, std::string* errorBuffer)
{
	CURLEx curlObj;
	if (!httpReuqestConfigure(&curlObj, request, writeFunc, writeStream, headerCallback, headerStream))
		return false;
	...
	if (!curlObj.setPostFields(request.data))
		return false;

	return curlObj.perform(curlCode, responseCode, errorBuffer);
}
//------------------------------------------------------------------------
bool httpPut(const HttpResuest& request, CURLEx_callback writeFunc, void* writeStream, CURLEx_callback headerCallback, void* headerStream, int* curlCode, int* responseCode, std::string* errorBuffer)
{
	CURLEx curlObj;
	if (!httpReuqestConfigure(&curlObj, request, writeFunc, writeStream, headerCallback, headerStream))
		return false;
...
//------------------------------------------------------------------------
bool httpDelete(const HttpResuest& request, CURLEx_callback writeFunc, void* writeStream, CURLEx_callback headerCallback, void* headerStream, int* curlCode, int* responseCode, std::string* errorBuffer)
{
	CURLEx curlObj;
	if (!httpReuqestConfigure(&curlObj, request, writeFunc, writeStream, headerCallback, headerStream))
		return false;
