...
function NetSocket_registerHandler(msgId, msgBuilder, msgHandler)
	...
	for key, val in pairs(mNetMsgHandlers) do
		if msgId == val.msgId then	-- 网络事件已注册
...
function NetSocket_unregisterHandler(msgId)
	for key, val in pairs(mNetMsgHandlers) do
		if msgId == val.msgId then	-- 事件已注册
			table.remove(mNetMsgHandlers, key)
			return
		end
	end
end
...
function NetSocket_getMsgName(msgId)
	for key, val in pairs(NetMsgType) do
		if msgId == val then
			return key
		end
	end
