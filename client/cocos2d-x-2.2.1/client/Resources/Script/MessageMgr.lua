...
----------------------------------------------------------------------
-- 初始化消息管理器
MessageMgr.init = function(historyMsgSize)
	mMsgTable = {}
...
----------------------------------------------------------------------
-- 插入一条消息
MessageMgr.insertMsg = function(priority, msgtype, times, data, limitCount)
	assert(priority > 0 and msgtype > 0 and times > 0, "MessageMgr -> insertMsg -> priority, msgtype, times must > 0")
	...
	LayerChat.pushChatNotice(createMsg(priority, msgtype, times, data))
end
----------------------------------------------------------------------
-- 弹出一条消息
MessageMgr.popMsg = function()
...
----------------------------------------------------------------------
-- 获取历史消息队列
MessageMgr.getHistoryMsg = function()
