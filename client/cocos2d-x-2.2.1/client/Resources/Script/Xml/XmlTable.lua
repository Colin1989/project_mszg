...
function XmlTable_load(fileName, keyName)
	assert(nil == mXmlFileName[fileName], "XmlTable -> load() -> repeat load file '"..fileName.."'")
...
function XmlTable_getRow(tb, key, dumpNil)
	...
	if nil == row then
		if true == dumpNil then
			assert(nil, "XmlTable -> getRow() -> can't find key '"..key.."' in file '"..tb.name.."'")
		end
		return nil
	end
	local temp = {}
