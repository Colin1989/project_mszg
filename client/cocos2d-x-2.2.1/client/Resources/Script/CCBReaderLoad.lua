    ...
    if nil ~= owner then
        ...
        for i = 1,ownerCallbackNames:count() do
            ...
            if "function" == type(owner[callbackName:getCString()]) then
                ...
                cclog("Warning: Cannot find owner's lua function:" .. ":" .. callbackName:getCString() .. " for ownerVar selector")
            end
        ...
        for i = 1,documentCallbackNames:count() do
            ...
            if "" ~= documentControllerName and nil ~= ccb[documentControllerName] then
                ...
                if "function" == type(ccb[documentControllerName][cbName]) then
                    ...
                    cclog("Warning: Cannot found lua function [" .. documentControllerName .. ":" .. callbackName .. "] for docRoot selector")
                end
            end
        end
...
function CCBReaderLoad(strFilePath,proxy,bSetOwner,strOwnerName)
    cclog("CCBReaderLoad was deprecated, Please use CCBuilderReaderLoad(strFilePath,proxy,owner) instead.In the newest CocosBuilderTest,you can find out the usage of this new function")
    return
end
