...
-export([update_pass_copy/2,update_pass_copy/3,query_all_pass_copy/0,query_all_pass_copy/1,
  ...
  get_need_stone/1, get_type/1]).

-export([create_push_tower/7,create_ranking_copy/7,get_disable_pos/1,get_next_key/1]).
...
create_ranking_copy(Mostters,Trap,Friend,Award,Scene,BarriesAmount,Enemy)->
    ...
    NewRandArray=RandArray--[AgainstPlayer],
    ...
    CurMap=#game_map{monster=Monsters,start=StartPos,key=0,barrier=Barriers,award=Buffs,trap=Traps,friend=Friends,scene=Scene,
		     enemy = [Enemy#senemy{pos=AgainstPlayer}]},
    CurMap.
...
get_allmap(MapId,_StartPos,Maps)->
    ...
    {Mons,KeyPos,Bars,Arr}=get_pos_by_rule(MapInfo#game_map_tplt.map_rule_id),
    RA = RandArray -- Arr,
    Key=case KeyPos of
	     0 ->
		 %%NextDoor=get_next_door(NextMap),
		 lists:nth(rand:uniform(length(RA)), RA);
		    %%,
		 %%create_door(StartPos,NextDoor,(RandArray--Arr)--get_disable_pos(NextMap));
		 %%create_door(RandArray--Arr--MapInfo#game_map_tplt.disable_pos);
	     _ -> KeyPos 
	 end,
    KeyMonster = get_key_monster(MapInfo#game_map_tplt.key_monster, Key),
    DoorPos = case MapId of
		  1 ->%%新手
		      6;
		  _ ->
		      create_door(Key, 0, RA -- [Key])
	      end,
    NewRandArray=((RA)--[Key])--[DoorPos],
    {Monsters,CurArray1}=create_monster(MapInfo#game_map_tplt.monster,Mons ++ KeyMonster,NewRandArray,[]),
    
    {Buffs,CurArray3}=create_award(MapInfo#game_map_tplt.buff,[],NewRandArray--CurArray1,CurArray1),
    ...
    CurMap=#game_map{monster=Monsters,start=DoorPos,key=Key,barrier=Barriers,award=Buffs,trap=Traps,friend=Friends,scene = MapInfo#game_map_tplt.room},

    get_allmap(NextMap,Key,[CurMap|Maps]).
...
get_clean_up_allmap(MapId, Maps) ->
  ...
  %%生成怪物
  KeyMonsterID = lists:nth(rand:uniform(length(MapInfo#game_map_tplt.key_monster)), MapInfo#game_map_tplt.key_monster),
  RuleMonsterIDs = case MapInfo#game_map_tplt.map_rule_id of
    0 ->
      [];
    _ ->
  ...
  CurMap = #game_map{monster = Monsters, award = Buffs},
...
get_pos_by_rule(Rule) ->
  case Rule of
    ...
    _ ->
      ...
      {Monsters, Door, RuleInfo#game_map_rule_tplt.barries_pos,
       RuleInfo#game_map_rule_tplt.monster_pos ++ RuleInfo#game_map_rule_tplt.barries_pos}
  end.
