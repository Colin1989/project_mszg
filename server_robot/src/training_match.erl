...
proc_req_train_match_settle(Packet = #req_train_match_settle{game_id = GameId, result = Result})->
	...
	{true,{{_, EnemyId, IsRobot}, MapInfo}} ->
	    ...
	    log:create(business_log, [player:get_role_id(), 0, 53, 5, erlang:localtime(), erlang:localtime(), 0]),
...
proc_req_get_train_match_times_info(Packet)->
    ...
    OrgTimes = config:get(max_train_times),
...
%%训练结算
settle_train(GameId, Result, EnemyId, IsRobot, _MapInfo)->
...
get_ranking_map_info(CopyInfo, TrainInfo)->	
    {#train_info{role_id = RoleId, level = Lev, type = Type}, IsRobot} = TrainInfo,
	...
	1 ->
	    game_copy:create_ranking_copy(Mostters,Traps,Friends,Awards,Scene,CopyInfo#ranking_copy_tplt.barrier_amount,
					  #senemy{type=Type,battle_prop=gen_robot_attr(Lev, Type)})
    end.
...
get_my_max_traintimes()->
    OrgMax = config:get(max_train_times),
    BuyTimes = get_buy_times(),
    OrgMax + BuyTimes.
...
gen_robot_info(Lev) ->
    ...
    {#train_info{role_id = uuid:gen(), level = Lev, power = Lev*100, type = rand:uniform(4), name = NickName, status = 0}, 1}.
