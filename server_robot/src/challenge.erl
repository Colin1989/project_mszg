...
proc_req_get_challenge_rank_award(#req_get_challenge_rank_award{}=Pack)->
	...
	0 ->
		...
		true ->
		    Point = do_get_ranking_award(MyRank),
		    packet:send(#notify_get_challenge_rank_award_result{result = ?common_success, point = Point});
		false ->
...
do_get_ranking_award(MyRank)->
    Point = get_ranking_award(MyRank),
    player_role:add_point(?st_get_rank_award, Point),
...
proc_req_challenge_settle(#req_challenge_settle{game_id=GameId, result=Result}=Pack)->
	...
	{true,{{_,EnemyId,EnemyRank,MyId,MyRank}, MapInfo}} ->
	    {Points, Honours} = settle_challenge(GameId,Result,EnemyId,EnemyRank,MyId,MyRank,MapInfo),
	    packet:send(#notify_challenge_settle{result=Result, point=Points, honour=Honours}),
...
proc_req_get_challenge_times_info(#req_get_challenge_times_info{}=Pack)->
    ...
    OrgTimes = config:get(max_challenge_times),
...
notify_enemy_be_challenge(Result,EnemyId,NewRank)->
    ...
    NewTimes = add_be_challenge_msg(EnemyId,Result,Name,NewRank),
...
get_challenge_award(_Rank,Result)->
    ...
    case Result of
	?game_win ->
	    {config:get(challenge_success_points), config:get(challenge_success_honours)};
	    %%{Award#challenge_award_tplt.success_points,Award#challenge_award_tplt.success_honours};
	?game_lost ->
	    {config:get(challenge_failed_points), config:get(challenge_failed_honours)}
...
settle_challenge(GameId,Result,EnemyId,EnemyRank,MyId,MyRank,_MapInfo)->
    ...
    {Points,Honours} = get_challenge_award(MyRank,Result),
    ...
    log:create(challenge_log, [GameId, Points, EnemyId, EnemyRank, MyRank, Result, Points, Honours, datetime:local_time()]), 
    {Points, Honours}.
...
get_my_max_challengetimes()->
    OrgMax = config:get(max_challenge_times),
    BuyTimes = get_buy_times(),
    OrgMax + BuyTimes.
